...
const memcachedAddress =
	process.env.ISUCONP_MEMCACHED_ADDRESS || 'localhost:11211'
...
app.use('*', async (c: AppContext, next: Next): Promise<void> => {
	...
	const sessionFromStore = existingSid
		? await getSessionFromStore(existingSid)
		: undefined
...
async function makePost(
	...
	const comments = await Promise.all(
		(commentRows as Comment[]).map(makeComment),
	)
...
async function render(
	...
	const html = await ejs.renderFile(path.join(__dirname, '../views', view), {
		...params,
		messages,
		formatBody,
	})
...
app.post('/login', async (c: AppContext) => {
	...
	try {
		const user = await tryLogin(
			ensureString(body.account_name),
			ensureString(body.password),
		)
		const session = c.get('session') as SessionData
		if (user) {
...
app.get('/:accountName{@[A-Za-z0-9_]+}', async (c: AppContext) => {
	try {
		...
		const commentCount =
			(commentCountRows[0] as CountRow | undefined)?.count ?? 0
...
app.post('/', async (c: AppContext) => {
	...
	const body = (await c.req.parseBody()) as ParsedBody
	const session = c.get('session') as SessionData
	if (body.csrf_token !== session.csrfToken)
		return c.text('invalid CSRF Token', 422)
	...
	let mime = file.type
	if (
		!(mime.includes('jpeg') || mime.includes('png') || mime.includes('gif'))
	) {
	...
	const [result] = await db.query<ResultSetHeader>(
		...
		[me.id, mime, buffer, ensureString(body.body)],
	)
...
app.get('/image/:filename{[0-9]+\\.(png|jpg|gif)}', async (c: AppContext) => {
	try {
		...
		if (
			...
			return new Response(new Uint8Array(post.imgdata), {
...
app.post('/comment', async (c: AppContext) => {
	...
	await db.query<ResultSetHeader>(
		...
		[Number(postIdString), me.id, ensureString(body.comment)],
	)
